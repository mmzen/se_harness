...
| 3. Work authorization | A work order selects requirements, specifications, applicable architecture/ADRs, and verification contracts; it defines scope, exclusions, execution permissions, stop conditions, and whether commit-bound verification is explicitly `required` or `not_required`. | An accountable owner records the assurance rationale; an engineering owner approves the bounded work. Approval is permission to start, not proof of completion. |
| 4. Execution and review | The coding agent runs start preflight, reads its manifest, sets the work order `in_progress`, implements within scope, runs repository checks, retains evidence, validates the selected scope and performs review preflight. | A person or agent executes within the approval grant without separate start, completion or preparation permission. Reviewers judge semantic fit and evidence quality. |
