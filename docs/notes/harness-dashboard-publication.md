...
Dashboards generated by releases up to 0.12.0 retain the exact unpkg dependency, CSP, timeout, and non-3D fallback accepted by `ADR-DST-008`. From 0.13.0 onward the Explorer is one self-contained document that names no remote origin (`ADR-DST-013`), so the published page requests only its own origin. Because the demonstration is generated by the root generator at the release integration commit, the 0.13.0 publication still rendered the previous page from its 0.12.0 root; the 0.14.0 publication, whose integration commit carried the 0.13.0 root, was the first to render the designed page, after `WO-DPG-002` taught the packager the designed page's notice boundary. GitHub Actions and GitHub Pages remain external availability and trust dependencies.
