...
When architecture applies, preflight also reads each selected architecture's decision assessment. An architecture that carries no `decision_assessment` is treated as exempt only when the graph declares it so: either it is a member of the frozen self-hosting set, or an approved work order names it in a `[definition_generation]` table. The architecture's own lifecycle status is not an input, so reaching `implemented`, `verified`, or `released` no longer stands in for an assessment. An exempt architecture with no selected active deciding ADR is reported as `W019`; the assessment it still owes is reported by validation as `W014` on every run, on the non-blocking `maintenance` plane. Preflight run through an evaluator that predates the resolver falls back to the frozen set alone and does not see a work-order declaration.
