...
At reviewed proposal revision `be8b4126`, all PLG records were draft or open. Subsequent decisions are recorded in each delivered artifact's metadata. This plan approves nothing.
...
Their definition and implementation changes may share a bounded PR after the separate start decision. Prior integration is not a start gate; the existing independent verification and integration decisions still apply.
...
Read-only start and review preflights on main `560973cf` plus the reviewed drafts reported those four inactive records for each probe, with no upstream or identity blocker. That diagnosis was not approval. Apply the operator's actual definition/WO approvals, rerun the current preflight, and record the separate start decision before implementation.

The owner authorized these two packets and their work after reviewing proposal `be8b4126`. Record approvals and starts through the evaluator on isolated branches based on current main `fd4584cc`. Codex and Claude use separate scopes; a stacked Claude branch avoids an early merge of Codex work.
