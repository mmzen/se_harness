...
The checkout and the locked root now report different versions, and they were always different identities. Candidate source in this checkout reports version 0.17.0, the version this candidate proposes to publish. The standard root installation remains governed by exact public 0.16.0, the `tool_version` recorded in `.engineering-harness.toml`, and both statements are true at the same time: the candidate version never replaces the root evaluator version, and neither replaces the other in this note. The schema-3 lock binds that released 0.16.0 version's immutable public wheel and installed-payload digests; checkout source includes later development changes and remains candidate evidence. A matching version string never granted verification, release, publication, deployment, or repository-upgrade authority, and a leading candidate version does not either — 0.17.0 is a proposal until a separately authorized release publishes it and a separately approved upgrade work order adopts it.
...
scripts/                                 repository-owned release support and maintenance scripts; no managed copy since the 0.16.0 root
...
The independent package lane runs the typed `qualify candidate-package` operation unconditionally. It once carried one bootstrap exception: exact public 0.6.0 predated the `qualify` namespace, so its digest-bound `accept-candidate` contract was accepted under its original `se-harness-functional-acceptance-v1` label. That path expired once every released verifier carried `qualify candidate-package` and was removed under `WO-REB-031` (released in 0.12.0); the retained 0.6.0 records keep their bytes. See [release qualification roles](release-qualification-roles.md).
...
- Latest markers, the step that was missed: `publish-pypi.yml` creates the GitHub Release with `--latest=false` on purpose and moves no alias tag, so a published release is *not* the latest release until the release owner makes it so. After the release contract's observation window, run `gh release edit vX.Y.Z --repo mmzen/se_harness --latest` and `git tag -f last vX.Y.Z && git push --force origin refs/tags/last`, then verify with `gh release list --json tagName,isLatest` (exactly the new tag `true`) and `git rev-parse last^{commit}` (equal to the record's `commit`). These two acts belong to every release, not to the workflow: `last` had stayed on a 0.7.1 commit through the 0.8.0 and 0.9.0 releases and the 0.9.0 Release stayed marked latest after 0.10.0 was published, until both were moved by hand on 2026-08-29. A release is finished when the record is `released`, the run is green, PyPI serves the bound digests, and both markers point at the new tag.
