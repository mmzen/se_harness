...
- Release contract with the WO-KIS-004 candidate: the owner chooses the final candidate and approves its work scope. `harnessctl release-unit . --from <tag> --to <commit>` is an advisory census; missing trailers need no exemptions. Capture and explicitly verify one final VREC covering all released work and required verification contracts, with evidence that the final integration was tested. Earlier records may have different commits and remain historical evidence. `prepare-release` binds that final VREC at its exact candidate. The installed 0.17.0 root retains its existing checks until a separately authorized upgrade.
