...
- `definition-lifecycle/`: approved definitions making a definition's lifecycle status answer only authority, moving generation to an explicit declaration and realization to derived work-order and verification coverage; WO-DLC-001 is implemented and verified by VREC-DLC-001, and the other two work orders remain draft.
