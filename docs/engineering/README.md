...
- `risk-management/`: approved definitions and bounded implementation work to make a threat to governed work a governed artifact (`RISK-`): one cause, one effect, a five-by-five score, stopped through a paired decision rather than a second gate, and closed only under verified coverage with a recorded residual.
...
- `release-0-13-0/`: aggregate qualification, provenance, and release records for version 0.13.0.
- `release-0-14-0/`: aggregate qualification, provenance, and release records for version 0.14.0.
