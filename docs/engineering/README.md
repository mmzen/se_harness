...
The distribution packet is under `harness-distribution/`. Commit-bound intent-to-revision traceability is governed by `revision-provenance/`. Draft aggregate multi-work-order release behavior is specified under `aggregate-release/`. The corrective portable managed-file integrity proposal is under `portable-managed-integrity/`. Templates and evidence are excluded from active-artifact parsing.
