...
Observe GitHub workflow inputs, environment deployment approval, logs, action SHA, release asset URLs and hashes, PyPI project/version/file pages, PyPI hashes and attestations, and an exact-version installation smoke test.
...
One manual job downloads and uploads one small wheel and sdist per release. No cache, persistent runner, background service, API-token rotation, or automatic retry is required.
...
Automation may validate inputs, download public release assets, compare bytes, request short-lived OIDC credentials through the pinned publisher, and upload only after environment approval. It may not choose a release, change hashes, approve deployment, retry a partial publication automatically, delete external state, or create a corrective version.

## Runbooks

Before dispatch, inspect the governing release record, tag target, GitHub release state, exact hashes, environment protection, PyPI publisher, and action pin. After success, compare PyPI hashes to the authorized inputs, inspect attestations, install `se-harness==VERSION` in a fresh environment, run `harnessctl --version`, and retain results. For any anomaly, stop and preserve external state.
