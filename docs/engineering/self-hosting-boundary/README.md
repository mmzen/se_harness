...
Normal `harnessctl upgrade` protects the implementation repository's `.engineering-harness.toml` and `.github/workflows/engineering-harness.yml`. The implemented `harnessctl reconcile-governor` is the separate plan-first mechanism for an authorized transition to an exact published target: it preserves field-owned repository policy, selects only role-correct self-hosting workflow material, requires explicit authority-bearing decisions, and changes the descriptor, controls, and lock through one recoverable transaction. The implementation delivered by this packet could not reconcile or govern its own creation release, and the command still grants no promotion authority by itself.
...
The repository's public releases and PyPI promotions were completed under separate accountable actions. External availability does not itself promote the independent self-hosting governor: `.self-hosting/governor.toml` selects released version 0.3.0 under `RLS-SEH-005`. Candidate version 0.4.1 remains a separate identity and cannot silently become the repository's governing baseline.
...
The correction is implemented and local source, package, migration, recovery, and Python 3.11 evidence is retained under `WO-SHB-002`. Normal root upgrade reports both repository-specific controls as `protected`. Aggregate assurance and release for that work are recorded by `VREC-SEH-005` and `RLS-SEH-005`; neither record authorizes a later governor promotion. Do not run root reconciliation or `harnessctl upgrade . --apply` without the distinct authority required for that operation.
