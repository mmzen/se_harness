...
- `WO-TST-004`: wave 4 of the code health assessment of 2026-09-07 (issue #379, section 5 and the wave 4 plan) under `REQ-TST-004` / `SPEC-TST-002` / `VER-TST-002`, drafted 2026-09-08: mixins for the 253 inherited re-runs, five support modules for the copied helpers, one retired-surface table for the tombstones, un-cited prose pins turned into structural checks, one fixture project name, the timings file on the hosted lane; product code stays with waves 2 and 3; no architecture relation. Approved 2026-09-08 ('Approve all four (Recommended)', PR #401, merged 13a70218) with one scope amendment for the packet's own definitions. Started 2026-09-08 on the owner's word 'start', before the wave 2 work orders merged; executes on `wo/tst-004-test-suite-hygiene` (PR #402), which took wave 2's merge (`4dc59d0a`) on 2026-09-08 and brought its three new test modules under the same rules.
