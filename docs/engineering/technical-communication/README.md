...
`WO-TCM-004` (2026-09-04) registers the `E-DCM` and `W-DCM` families that `WO-DCM-001` added to the validator, under the existing `REQ-TCM-005`, `SPEC-TCM-002` and `VER-TCM-002`, regenerates the index, and adds the unregistered-family guard that closes the residual uncertainty `VER-TCM-002` recorded: a hyphenated diagnostic family present in the source but absent from the registry now fails `--check` and the pinning test.
