...
7. **Two paths outside the execution scope were edited and reverted before the
   handoff.** `docs/notes/harnessctl-reference.md` (the `--checkpoint
   transition --target` preview) and `SPEC-WEX-002` (its line naming
   `se-harness-quality-gates-v1`, which needs a dated amendment to `v2`) are
   not in `WO-ECP-009`'s `[execution_scope]`; `QGP-G4I-PATHS` refused the
   first handoff run on the reference note. Both edits were withdrawn; the
   note can follow on the ungoverned path and the specification amendment
   needs the owner's scope amendment or a later work order. Until then
   `SPEC-WEX-002` names the retired contract version.

## 7. Handoff checkpoint binding

artifact: WO-ECP-009
checkpoint: handoff
formal_snapshot_sha256: c5d1ab614d2b8e5cd8bb49a60b486a55183b3d3b4070a6fa2c666e1bfdad4069
