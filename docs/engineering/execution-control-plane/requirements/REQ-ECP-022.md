...
created = "2026-08-29"
updated = "2026-08-29"
statement = "WHEN an actor asks the evaluator what applies to one selected artifact, THE SYSTEM SHALL answer through `harnessctl check`, which without a checkpoint projects the selected rule, procedure and next step and with a checkpoint additionally evaluates its gates, so that one command and one rule selection serve both questions."
