...
- `WO-ECP-028`, `WO-ECP-029`, `WO-ECP-030` independent of one another; wave 1 deletions under `REQ-ECP-033` / `SPEC-ECP-022` / `VER-ECP-024`, drafted 2026-09-07 from issue #376: group A dead code and hygiene, group B the `adopt` alias, group C the dormant surface and the two unreachable contract entries (the latter edits candidate template copies, felt on this root at the next adoption); no architecture relation. Approved 2026-09-07 ('i approve', PR #385); each awaits the owner's merge and start decision.
