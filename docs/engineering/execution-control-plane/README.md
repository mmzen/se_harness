...
- `WO-ECP-034`, `WO-ECP-035`, `WO-ECP-036` in order, the first unblocking the other two; wave 3 engine under `REQ-ECP-035` / `SPEC-ECP-024` / `VER-ECP-026`, drafted 2026-09-08 from issue #378 (code health assessment 2026-09-07, sections 2.1, 2.2, 3.6 and the wave 3 plan) and the revisit triggers of `DEC-ECP-001` and `DEC-ECP-002`: group A the engine as an import surface and its five twins folded into the package, group B one validation per governance command, group C the validator, the generator and the compliance module split along their seams; `SPEC-DST-025` receives the amendment record for `DST-ENG-003` and `DST-ENG-006` under group A; no architecture relation.
