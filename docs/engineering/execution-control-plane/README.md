...
- `WO-ECP-031`, `WO-ECP-032`, `WO-ECP-033` independent of one another; wave 2 primitives under `REQ-ECP-034` / `SPEC-ECP-023` / `VER-ECP-025`, drafted 2026-09-08 from issue #377 (code health assessment 2026-09-07, section 2.1 and the wave 2 plan) and the owner decision of issue #381 item 4: group A the process launcher and the front-matter parser, group B the integrity primitives, the closed sets and the two grammars, group C the diagnostic-code registry and the four contract tables read at run time; the engine's copies stay for wave 3 (#378); no architecture relation.
