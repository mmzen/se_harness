...
**`ECP-PRM-027` is met as written and the deviation `DEC-ECP-002` accepted against it is discharged, recorded 2026-09-09 under `WO-ECP-037`.** `DEC-ECP-002` accepted, for wave 2, that the duplication scan still reported three cross-file blocks, each with a side inside `se_harness/engine/`, and named the merge of wave 3 (#378) as its revisit. Wave 3 merged on 2026-09-08: `WO-ECP-034` folded the three engine copies into the package and its verified record `VREC-ECP-038` retains the scan (`pylint --enable=duplicate-code --min-similarity-lines=8`) at 0 blocks, a reading `WO-ECP-035` and `WO-ECP-036` repeated at their heads. The rule stands as written and is met; no successor decision is needed, because the rule did not change and nothing departs from it. Nothing else in this specification changes.
