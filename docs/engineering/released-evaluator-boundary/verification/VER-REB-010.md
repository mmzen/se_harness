...
| `REQ-REB-024` | junction predicate test | `mklink /J` parent with `is_symlink()` false and `is_junction()` true; and a stubbed runtime without `is_junction` | The junction parent is refused; the missing predicate refuses rather than passes |
...
3. A junction parent is refused on Windows. The test asserts independently that `is_symlink()` is false and `is_junction()` is true for that parent, so the case cannot pass through symbolic-link detection alone.
4. A runtime without `is_junction` refuses with the unavailable-predicate case rather than skipping the check.
