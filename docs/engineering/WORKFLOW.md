...
6. Retain evidence keyed to every release-bearing work-order ID, run `harnessctl preflight . --work-order WO-... --phase review`, inspect current attention with `harnessctl inspect .`, generate Harness Explorer with `harnessctl dashboard .`, and review the candidate's consistency and anomaly findings.
