...
- `REQ-CIP-008`, `REQ-CIP-009`, `SPEC-CIP-003`, `VER-CIP-003`, `WO-CIP-007` (drafted 2026-09-08 for issue #380, wave 5 of the code health assessment of 2026-09-07): the rehearsal's candidate leg replays the recipe and stops repeating the qualification and the suite that `candidate-evidence` already ran on the same commit; the two inline re-checks, the dead payload-flag probe and the two retired names (`governor`, `governance-migration`) leave the workflows; one pin form, one Python version and one set of integration tool versions; the Pages deployment queues behind one group; the manifest script stops producing schema-1 bundles. The managed template is `WO-DST-026`'s; delegated route.
