...
- `REQ-RLO-017`: present the committed bytes and one declared file-mode set to the producer on every host platform. GitHub issue [#189](https://github.com/mmzen/se_harness/issues/189) records `RC-070-01`: the 0.7.0 build of record was produced on a Windows workstation, agreed with itself across both producer instances, and was wrong in 83 line-ending and 69 mode facts, which cost the rejection of `RLS-SEH-014`. The host boundary `ARCH-RLO-004` already declares was not achieved by the implementation, and no comparison inside a replay can see the difference.
- `WO-RLO-008`: disable Git line-ending conversion on the candidate export and establish the declared mode set inside the producer. Drafted and approved 2026-08-27 in one act with `REQ-RLO-017` and the `SPEC-RLO-004`, `ARCH-RLO-004`, and `VER-RLO-004` amendments framed in the work order, because the requirement has no coverage without them. The issue asks instead that a Windows host be refused; the owner declined any non-POSIX refusal or warning and the change of intent is recorded against the issue. Approved and started on 2026-08-27, with the change built in one commit at the owner's direction, against the repository's one-commit-per-act convention, and marked `implemented` the same day under `DR-WO-COMPLETE`. A Windows replay of released `RLS-SEH-015` at that commit reproduces its bound wheel and sdist exactly, and removing either half of the response in turn reproduces neither, which is retained under `evidence/WO-RLO-008/`. The completion decision was taken with that evidence's deviation 3 in view: no POSIX host reading and no hosted `candidate` job exists, and the two mode-reading tests are skipped on Windows, so the POSIX half of `VER-RLO-004`'s host-independence row was owed to the commit-bound verification. Verified `VREC-RLO-008` discharges that obligation: it binds the retained evidence to the completion commit, accepts all five deviations unsoftened, and records in its decision the hosted reading that substantially closes deviation 3 — pull request #202's rehearsal supplied this producer code to an `ubuntu-latest` replay of `RLS-SEH-016`'s bound candidate, reaching that record's bound wheel and sdist exactly, which is not the `RLS-SEH-015` the row names. Release, publication, and every external action remain separate decisions.
