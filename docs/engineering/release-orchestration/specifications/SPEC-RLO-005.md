...
Amendments `A8` and `A9` came later and from a different cause: `main` advanced under the open branch and changed the orchestrator itself, rather than a mismeasurement in this packet. They are not covered by the acceptance above, so they were put separately, and the accountable repository owner accepted both on 2026-08-24 through the statement `Accept A8 and A9`. The framing that decision was taken over was: "A8: resolve a job's platforms from its matrix and move the platform claim from the job to the step, deriving each mechanic's claim as the union over the steps that realize it; declaration schema advances to v2 with four new load-time refusals. A9: rule 1 names twenty-two mechanics, and new rule 40 declares the recipe-bound replay always `excluded` with a measured reason in fixed precedence." That acceptance covers rules 38 to 40, declaration schema `v2`, the twenty-second mechanic, and the consequent entries in the state model, the outputs, the error and recovery table, and the examples. Neither amendment changes the authority boundary, and the acceptance authorizes no release, publication, deployment, merge, or other external action.
