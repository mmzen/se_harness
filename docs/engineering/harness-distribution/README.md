...
Wave 5 managed template hygiene:

`INT-DST-001` -> `CAP-DST-001` -> `REQ-DST-072..075` -> `SPEC-DST-027`, `VER-DST-027` -> `WO-DST-026`.

Drafted 2026-09-08 for issue #380 (code health assessment 2026-09-07, section 4). The managed workflow's two check steps hand an empty result to a JSON reader when the evaluator refuses, so a guard refusal surfaces as a decoder traceback; its header still describes doctor and validate steps it no longer runs; its three actions float on major tags; the installer marks the `.gitignore` block with HTML comments, which gitignore reads as two patterns; and `SE_HARNESS_REHEARSAL`, read by the gate source, is named in no specification. `WO-DST-026` fixes the template and the installer's fragment writer, amends `SPEC-ECP-006` by record, and pins the environment inventory. Every root managed byte of this repository changes only at the root adoption of the carrying release; `WO-DST-025`'s inert-key removal already covers the configuration half of the issue. Delegated route; approved 2026-09-08 and executed the same day on `wo/dst-026-managed-template`: the template's two check steps capture status and stderr, its header names the eight steps it runs, its three actions are pinned to commits, `installer._block` writes the `.gitignore` block between hash markers, and `tests/test_managed_template_hygiene.py` pins the rules and the environment inventory (three names read under `se_harness/`, `SE_HARNESS_REHEARSAL`, `GITHUB_TOKEN` and `PYTHONPATH`, each named in a specification). The root adoption obligation `DST-MWF-014` is carried to the root-adoption work order of the carrying release. Evidence: `evidence/WO-DST-026-verification.md`.


Issue #433 managed-template leftovers:

`INT-DST-001` -> `CAP-DST-001` -> `REQ-DST-076..077` -> `SPEC-DST-028`, `VER-DST-028` -> `WO-DST-027`.

Drafted 2026-09-10 for issue #433 (findings 1 and 3). Rule `TRC-008` of the managed `TRACEABILITY.md` still calls `ARCH.constrains` compatibility-only and promises a classification and a migration report, while the validator has refused every `constrains` relation with `E016` since `WO-AUT-006`; the owner kept the template out of that work order (`SPEC-AUT-004` `AUT-WIN-018`). The "Completion report format" heading of the managed `WORK_ORDER.template.md` carries no guidance, so 79 work orders copied "the completion decision is the engineering owner's" from one another, 14 of them under `[delegation] class = "execution"`, which gives that decision to the delegated executor while the required check is green. `WO-DST-027` rewrites `TRC-008` for the retired relation, puts one guidance sentence under the template heading naming who decides completion, corrects the sentence of `docs/notes/harness-uml-model.md`, declares both changes in the parity test and pins them. The historical work orders are records of their time and are not rewritten. Every root managed byte of this repository changes only at the root adoption of the carrying release. The two `ci-pipeline` definitions of finding 2 are `WO-CIP-008`'s. Delegated route; executing on `wo/dst-027-managed-templates`.
