...
5. For an exact-output request, stop communication rendering and follow the
   existing exact-output path. If its governing procedure requires a canonical
   block alone, return that block unchanged, without result/receipt wrappers or
   a managed-profile claim; do not continue to steps 6–7. For eligible prose,
   compose one brief under `operator-communication`. Lead with the outcome or
   one required action. Preserve the accountable actor, conditions,
   qualifications, normative force, and one canonical next action.
