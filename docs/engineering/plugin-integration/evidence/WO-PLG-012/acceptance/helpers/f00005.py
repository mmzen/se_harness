...
def main():
    ...
    parser.add_argument("--expected-payload", help="Explicit identity probe only; never changes the helper or a target lock")
    ...
    if args.expected_payload and args.operation != "identity":
        parser.error("--expected-payload is for direct identity probes only")
    identity = ["identity", "--role", "released-evaluator", "--expected-version", args.expected_version, "--expected-root", expected_root,
                "--checkout-root", str(target), "--require-isolated-python", "--evaluator-payload-sha256", args.expected_payload or lock["evaluator"]["payload_sha256"],
    ...
    config["evaluator_commands"] = [["--version"], identity, ["--help"], ["doctor", str(target)], ["validate", str(target), "--json"],
                                    ...
                                    ["check", str(target), "--artifact", "WO-ROF-001", "--json"],
