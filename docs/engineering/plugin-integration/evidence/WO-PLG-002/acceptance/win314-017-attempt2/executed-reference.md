...
The probe checks `venv` and `ensurepip` imports and the available pip version.
Environment creation then exercises offline pip bootstrapping. Disk or creation
failures remain incomplete setup. Do not assume where a Python distributor
stores its bundled wheels.
