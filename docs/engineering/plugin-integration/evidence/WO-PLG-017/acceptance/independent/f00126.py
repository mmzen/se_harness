...
def main():
    ...
    def git(label, *args, target=None, expected=0):
        ...
        return call(label, ['git', '-c', 'safe.directory='+source.as_posix(), '-c', 'safe.directory='+repo.as_posix(),
    ...
    try:
        ...
        git('03-clone', 'clone', '--no-checkout', '--no-hardlinks', '--no-tags', '--single-branch', str(source), str(repo), target=source)
