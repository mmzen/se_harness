...
$probeRuntimeFile = Join-Path $probeData 'runtime-path.txt'
if (Test-Path -LiteralPath $probeRuntimeFile) {
    $probePython = (Get-Content -LiteralPath $probeRuntimeFile -Raw).Trim()
    if (Test-Path -LiteralPath $probePython -PathType Leaf) {
        $probeInput | ConvertTo-Json -Compress | & $probePython -I (Join-Path $env:PLUGIN_ROOT 'observe_runtime.py')
        exit $LASTEXITCODE
    }
}
