...
implemented. This packet records only WO-PLG-004's transition, applied by the
...
before this decision. The managed CI workflow checks scope in every state and
recomputes the implementation handoff only while a work order is `in_progress`.
