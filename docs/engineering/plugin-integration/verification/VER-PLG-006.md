...
Use SPEC-PLG-006, DEC-PLG-002, and retained WO-PLG-004 observations for expected host behaviour.
...
| --- | --- | --- | --- |
| REQ-PLG-009 | Test, demonstration, inspection | Accepted activation; malformed event; absent binding; script failure | Shared skills and scripts run through accepted bindings; invalid or incomplete integration never reports readiness. |
...
Activate the production package in a disposable Claude Code profile.
...
Execute every positively accepted Windows, Linux, or macOS combination recorded through DEC-PLG-002. An empty supported set cannot pass adapter acceptance.
Name exact host, Python, and released-evaluator versions; list unaccepted combinations separately.

## Evidence retention

Retain package identity, event inputs/outputs, observed invocation arguments, and results under `evidence/WO-PLG-006/`.
