...
id = "VER-PLG-006"
type = "verification"
title = "Claude Code adapter conformance to accepted activation evidence"
status = "draft"
...
[relations]
verifies = ["REQ-PLG-009"]
+++

# Verification Contract: Claude Code adapter conformance to accepted activation evidence

## Independence

Expected mappings come from SPEC-PLG-006, DEC-PLG-002 and the retained WO-PLG-004 host proof. Expected script effects come from SPEC-PLG-007/008. Fix timing budgets and expected target effects before execution; an independent observer records target hashes and effect counts outside hook logs.

## Requirement-to-evidence matrix

| Requirement | Method | Cases | Pass condition |
| --- | --- | --- | --- |
| REQ-PLG-009 | test, inspection | C01–C12 | Accepted discovery/invocation mapping and timely refusal hold; missing required enforcement or invalid timing refuses qualification. |

## Acceptance scenarios

Each case creates new test evidence under `evidence/WO-PLG-006/Cnn/`: `actions.txt`, `stdout.txt`, `stderr.txt`, and `observations.json`; these are not plugin APIs.
...
Retain a mapping from each host binding to its shared source under ARCH-PLG-002/ADR-PLG-002; no copied evaluator policy is accepted. C09–C12 verify PLG-CLCA-009/010 against actual host bindings; C12 also rejects asynchronous enforcement.
...
Run every positively accepted Claude Code/OS combination with recorded host, Python and evaluator versions. C09–C12 repeat the shared failure cases on real bindings; protocol fixtures alone cannot satisfy live-host evidence.

## Evidence retention

Retain each case’s fixture revision, argv, exit status, raw output and listed observations under `evidence/WO-PLG-006/Cnn/`.
