...
- Release preparation: pass the retained bundle to `harnessctl prepare-release ... --distribution-manifest <bundle.json>`; historical RLS files may omit the block, but the SE Harness publication orchestrator requires it.
- Authorized last mile: after the RLS is `released` in `main`, dispatch `.github/workflows/publish-pypi.yml` from `main` with only `release_record=RLS-...`. The workflow derives the candidate, governance commit, version, tag, files, and hashes; rebuilds twice without credentials; reconciles GitHub and PyPI immutable state; deploys the Pages demonstration from the main-history governance snapshot; and reports stage-specific outcomes. The protected `pypi` environment remains a separate human decision.
...
- Major components and responsibilities: `se_harness/` is the safe control plane, including runtime identity, governor reconciliation, verifier-owned candidate acceptance, read-only preflight, and release-distribution manifest validation; `templates/repository/standard/` is the one canonical consumer installation; `self_hosting/` contains published data-only migration and role-specific workflow material and is not an installation profile; `.self-hosting/governor.toml` selects the exact released governor for this implementation repository; `.github/workflows/engineering-harness.yml` is the active repository-specific gate, `.github/workflows/self-hosting-governor.yml` is candidate reusable workflow material until publication and later promotion, `.github/workflows/publish-pypi.yml` is the repository-specific one-input release orchestrator and stable PyPI Trusted Publisher identity, and `.github/workflows/publish-dashboard-pages.yml` is main-only Pages recovery; `.github/scripts/publish_release.py` resolves and reconciles last-mile state; `scripts/` validates, selects structured CI work orders, renders the local Explorer, and contains deterministic release-build support; `tests/` verifies installer, upgrade, reconciliation, acceptance, diagnostics, instruction routing, preflight, CI, provenance, and deterministic distribution behavior; `docs/engineering/` contains the governing artifact graph.
