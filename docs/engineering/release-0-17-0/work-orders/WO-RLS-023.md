...
rationale = "Release, package installation, the following evaluator adoption, recipe-bound build replay and credential-free publication rely on the exact candidate, its retained evidence and its reproducible build; every consumer that upgrades to 0.17.0 receives the command surface, the exit codes, the risk artifact, the engine relocation and the templates this candidate carries."
decided_by = "release-owner"

[execution_scope]
paths = ["docs/engineering/README.md", "docs/engineering/release-0-17-0/", "docs/notes/developing-se-harness.md"]
...
from = "in_progress"
to = "implemented"
decided_at = "2026-09-09T07:56:31Z"
decided_by = "engineering-owner"
reason = "Marked implemented by the accountable engineering owner on 2026-09-09 under DR-WO-COMPLETE, by selecting the presented option 'Complete and prepare the record (Recommended)': every reading of REL-SEH-028's candidate-qualification section is recorded in the evidence packet with its evaluator and platform, the census from v0.16.0 is complete with the seven exemptions the contract names and WO-TST-004 traced through the trace commit 10e4face, the hosted build reading at 10e4face was byte-identical twice on the pinned producer (run 34321911662), every hosted lane at 308b2304 on both events is success, and the Git-derived handoff check completes with every predicate passing. This commit is the candidate the records bind."
+++
...
preflight or any declared work, after `REL-SEH-028` is approved by the
...
Produce the one clean 0.17.0 candidate commit on a branch off `main` at the
re-measured census commit or later, prove the candidate with the governing
0.16.0 evaluator and the candidate's own qualification, take the build of
...
already read 0.17.0 (`WO-HUP-017`). The candidate commit is therefore the
commit that retains this work order's evidence and the index maintenance;
its packaged bytes are those of `main`.

## Aggregate scope

The census this work order carries is `REL-SEH-028`'s as re-measured at
...
`harnessctl release-unit . --from v0.16.0 --to <candidate> --contract REL-SEH-028`
is re-run and recorded; a difference beyond the released `WO-RLS-022`
traced through the #369 merge and this work order itself is a stop
condition.

## In scope

1. The trace repair: one empty commit on the release branch carrying
   `Harness-Work-Order: WO-TST-004` in its final block, so the derivation
   reads `WO-TST-004` through the first-parent path, the execution merge
   #402 having carried no trailer; it is the one commit of this branch
   whose trailer is not this work order's, it changes no path, and the
   contract names it.
2. Qualification at the candidate, governing 0.16.0 evaluator outside the
   ...
   upgrade rehearsal 0.16.0 to 0.17.0 on both hosted platforms at the
   candidate head.
3. Build of record from the hosted Publication Rehearsal in `candidate`
   mode dispatched on `release/0.17.0` at the bound candidate: two
   ...
   retained as
   `docs/engineering/release-0-17-0/evidence/RLS-SEH-026-bundle.json` when
...
4. Evidence: this work order's handoff packet under
   `docs/engineering/release-0-17-0/evidence/WO-RLS-023/` with the formal
   snapshot, every reading above, the census at the candidate, the hosted
   lanes at the candidate head, and any deviation.
5. Index maintenance: `docs/engineering/README.md` domain line,
   `docs/engineering/release-0-17-0/README.md`, and the release note in
   `docs/notes/developing-se-harness.md` if a sequence fact changed.

## Out of scope

- Approval of `REL-SEH-028`; preparation, verification or transition of
  `VREC-SEH-026` and `RLS-SEH-026`; the tag, the GitHub Release, PyPI,
...
- Any change under `se_harness/`, any managed path, any template, any
  workflow, any test.
- The adoption of 0.17.0 as this repository's root; the start of
  `WO-TCM-011`.
...
- Every commit on the release branch carries the
  `Harness-Work-Order: WO-RLS-023` trailer in its final block, except the
  one empty trace commit of item 1, so the census at the candidate needs no
  exemption beyond the six trailer-less merges the contract names.
...
- No promotable distribution leaves this work order; the bundle manifest is
  retained evidence until `RLS-SEH-026` binds it.

## Expected change surface

The domain packet, this work order's evidence, two index files, one empty
trace commit and possibly one note line.

## Required verification

Execute the candidate-qualification and build-of-record sections of
`REL-SEH-028` in full; repository-required checks; the pull request's
lanes; the handoff check over the Git-derived change set.

## Evidence to record

`docs/engineering/release-0-17-0/evidence/WO-RLS-023/`.

## Stop and escalate conditions

Any work order reaching `implemented` with packaged-surface bytes after
`REL-SEH-028`'s approval; `WO-TCM-011` leaving `approved`; any census
difference beyond the released `WO-RLS-022` and this work order; a hosted
producer run that is not byte-identical to its twin; any hash-locked file
in the change set.
