...
Uses only generated request schemas. Optional setup approval answers are one-shot
responses for exact commands under existing operator authority. It never grants
hook trust, persistent permissions, exposes login output, or simulates events.
"""
import argparse
import json
import os
...
from static_profile_inventory import snapshot as static_profile_snapshot
...
def main():
    ...
    parser.add_argument('--inventory-static-profile', type=Path, help='Metadata only for the named normal-profile config/auth/plugin inputs')
    ...
    tool_environment = sandbox / 'profile/codex/plugins/data/codex-probe-codex-probe-local' / args.tool_environment_name
    ...
    if args.inventory_static_profile:
        outcome['normal_profile_static_before'] = static_profile_snapshot(args.inventory_static_profile)
    if args.tool_setup_probe or args.inventory_static_profile:
    ...
    try:
        ...
        if args.setup_skill_missing or args.tool_setup_probe or args.inventory_static_profile:
            outcome['repository_after'] = {p.relative_to(sandbox / 'repo').as_posix(): digest(p)
                for p in (sandbox / 'repo').rglob('*') if p.is_file()}
        if args.inventory_static_profile:
            outcome['normal_profile_static_after'] = static_profile_snapshot(args.inventory_static_profile)
