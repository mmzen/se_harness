...
class StaticMetadataTests(unittest.TestCase):
    def test_named_metadata_only_never_reads_contents_or_session_records(self):
        with tempfile.TemporaryDirectory(prefix='codex-static-metadata-') as folder:
            ...
            self.assertEqual(observed['files']['config.toml'], {'exists': False, 'absence_confirmed': True})

    def test_stat_denial_is_unknown_not_absent(self):
        with tempfile.TemporaryDirectory(prefix='codex-static-denial-') as folder:
            original = Path.stat
            def stat_path(path, *args, **kwargs):
                if path.name == 'auth.json':
                    raise PermissionError('synthetic access denial')
                return original(path, *args, **kwargs)
            with patch.object(Path, 'stat', stat_path):
                observed = snapshot(folder)
            self.assertIsNone(observed['files']['auth.json']['exists'])
            self.assertEqual(observed['files']['auth.json']['error'], 'PermissionError')
