...
def main():
    ...
    ap.add_argument("--create-repository-by-replay", action="store_true")
    ap.add_argument("--command-timeout", type=float, default=60)
    ns = ap.parse_args()
    if not sys.flags.isolated:
        ap.error("Use the verified external evaluator Python with -I")
    if not 0 < ns.command_timeout < float("inf"):
        ap.error("--command-timeout must be finite and positive")
    ...
    elif ns.create_repository_by_replay:
        if not output.is_dir() or repo.exists():
            raise ValueError("Recorded clone requires an existing output and absent target")
    else:
    ...
    bindings = {"{python}": python, "{environment}": str(Path(python).parent.parent),
                "{repo}": str(repo), "{bundle}": str(bundle), "{trace}": str(traces), "{output}": str(output)}
    env = os.environ.copy()
    ...
    try:
        for record in manifest["records"]:
            ...
            row = {"label": label, "argv": argv, "stdin": stdin, "cwd": str(output), "exit_code": exit_code,
            ...
            if command_error is not None:
                row["error"] = {"type": type(command_error).__name__, "message": str(command_error)}
                row["timed_out"] = isinstance(command_error, subprocess.TimeoutExpired)
            path.write_text(json.dumps(row, indent=2) + "\n", encoding="utf8", newline="\n")
            summary["commands"].append({"label": label, "argv": argv, "exit_code": exit_code})
            print(json.dumps({"label": label, "exit_code": exit_code, "non_git_changes": [p for p in changed if not p.startswith(".git/")]}), flush=True)
            if command_error is not None:
                raise command_error
            if exit_code != record["expected_exit"]:
