...
def main():
    ...
    if not 0 < args.command_timeout < float("inf"):
    ...
    try:
        ...
        for record in manifest["records"]:
            ...
            row = {"label": record["label"], "argv": argv, "cwd": str(output), "exit_code": exit_code,
            ...
            if command_error is not None:
                raise command_error
            if exit_code != record["expected_exit"] or changed != record["expected_changed_paths"] or local_before != local_after:
            ...
            print(json.dumps({"label": record["label"], "exit_code": exit_code, "changed_paths": changed}), flush=True)
        summary["passed"] = True
