...
def ready_context_observed(record, events, debug):
    """Require readiness and context receipt in this exact host invocation."""
    if not live.completed_successfully(record):
        return False
    for event in events:
        if event.get('event') != 'SessionStart' or not handler_readiness(event):
            continue
        observation = json.loads(event['handler_output'])
        context = event.get('host_output', {}).get('hookSpecificOutput', {}).get('additionalContext')
        if not isinstance(context, str) or not context or context != observation.get('context'):
            continue
        receipt = 'provided additionalContext (' + str(len(context)) + ' chars)'
        if any('Hook SessionStart (' in line and receipt in line for line in debug.splitlines()):
            return True
    return False
...
def main():
    ...
    for source in [Path(__file__), probe.HERE / 'probe.py', probe.HERE / 'inline_probe.py', probe.HERE / 'live_sessions.py']:
    ...
    def invoke(name, prompt, allowed=None):
        ...
        if debug.exists():
            ...
            debug_text = debug.read_text(encoding='utf-8', errors='replace')
            context_receipts.append({'step': name, 'recognized_additional_context':
                                     'provided additionalContext' in debug_text})
        ...
        return result, observed, debug_text
    ...
    setup_record, setup_events, setup_debug = invoke('after-setup-start', 'Say setup attempt recorded. Do not use tools.')
    ...
    ready_to_remove = not args.ready_fixture or ready_context_observed(setup_record, setup_events, setup_debug)
