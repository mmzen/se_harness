...
class ProcedureError(CodedError):
    """A typed procedure or parameter cannot resolve safely; every refusal carries its code."""
...
def _typed(value: str, parameter_type: str, name: str) -> str:
    ...
    if parameter_type == "path":
        ...
        if value.startswith("/") or "\\" in value or ":" in value or any(part in {"", ".", ".."} for part in parts):
            raise ProcedureError(WEX221, f"procedure parameter {name} is not a normalized path")
    if parameter_type == "status" and re.fullmatch(r"[a-z][a-z0-9_]*", value) is None:
        raise ProcedureError(WEX221, f"procedure parameter {name} is not a status")
    return value
    ...
    for parameter in procedure.get("parameters", []):
        ...
        if cardinality == "one":
            if not isinstance(value, str) or not value:
                raise ProcedureError(WEX221, f"required procedure parameter {name} is unavailable")
            result[name] = _typed(value, parameter_type, name)
        elif cardinality == "zero_or_one":
            if value is not None and (not isinstance(value, str) or not value):
                raise ProcedureError(WEX221, f"optional procedure parameter {name} has an invalid value")
            ...
            if not isinstance(value, (list, tuple)) or not value or not all(isinstance(item, str) and item for item in value):
                raise ProcedureError(WEX221, f"repeated procedure parameter {name} is unavailable")
...
def _expand_text(template: str, values: Mapping[str, Any]) -> str:
    def substitute(match: re.Match[str]) -> str:
        ...
        if isinstance(value, list):
            raise ProcedureError(WEX221, f"repeated parameter {match.group(1)} must occupy one argv element")
        if value is None:
            raise ProcedureError(WEX221, f"procedure parameter {match.group(1)} is unavailable")
        return str(value)
...
def resolve_procedure(
    ...
    if procedure is None:
        raise ProcedureError(WEX220, f"unknown procedure {procedure_id}")
...
def corrective_response(
    ...
    *,
    ...
    if form is None:
        raise ProcedureError(WEX_ADS_001, f"step {step.get('id')} has no corrective form for {predicate_id}")
