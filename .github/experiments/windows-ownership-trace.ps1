param([ValidateSet('disk','ramdisk')][string]$Storage,
      [ValidateSet('baseline','chunked')][string]$Variant)
$ErrorActionPreference = 'Stop'
$env:RAMDISK_TESTED_COMMIT = if ($Variant -eq 'baseline') { $env:RAMDISK_BASELINE_COMMIT } else { $env:GITHUB_SHA }
$env:READ_VARIANT = $Variant
$root = if ($Storage -eq 'ramdisk') { "R:\ownership-trace-$Variant" } else { Join-Path $env:RUNNER_TEMP "ownership-trace-$Variant" }
...
$tracePath = Join-Path $env:RUNNER_TEMP "$Storage-$Variant-ownership.etl"
$wpr = Join-Path $env:SystemRoot 'System32/wpr.exe'
$profile = (Join-Path $scripts 'Ownership.wprp') + '!Ownership'
$instance = "Ownership-$Storage-$Variant"
Set-Location -LiteralPath $source
& (Join-Path $runtime 'python.exe') -I -B (Join-Path $scripts 'test_bounded_read.py') $source
if ($LASTEXITCODE -ne 0) { throw 'Reader boundary checks failed' }
Write-Host "TRACE_START $Storage $Variant"
...
Write-Host "TRACE_ANALYSIS $Storage $Variant TEST_EXIT=$testStatus"
...
Write-Host "TRACE_DONE $Storage $Variant TEST_EXIT=$testStatus"
